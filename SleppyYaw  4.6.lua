...
local Yaw_Limit_Min = menu.SliderInt("SleepyYaw","Random - Min", 0, 0, 60)
local Yaw_Limit_Max = menu.SliderInt("SleepyYaw","Random - Max", 0, 0, 60)
local Lagsync_speed = menu.SliderInt("SleepyYaw","Random - YawAdd", 0, 0, 60)
Yaw_Limit_Min:SetVisible(false)
Yaw_Limit_Max:SetVisible(false)
Lagsync_speed:SetVisible(false)
local text = menu.Text("SleepyYaw", "--------------------------------------------------")
local Desync_Indica = menu.Switch("SleepyYaw", "Indicator", false, "")
local Desync_Text_Outline = menu.Switch("SleepyYaw","Text Outline", true, "")
local Desync_Info_Col_Top = menu.ColorEdit("SleepyYaw","Top Color",Color.new(1.0, 1.0, 1.0, 1.0),"")
local Desync_Info_Col_bottom = menu.ColorEdit("SleepyYaw","Bottom Color",Color.new(1.0, 1.0, 1.0, 1.0),"")
local Desync_Info_Col_middle = menu.ColorEdit("SleepyYaw","Middle Color",Color.new(1.0, 1.0, 1.0, 1.0),"")
local Desync_Col_Text = menu.ColorEdit("SleepyYaw","Text Color",Color.new(1.0, 1.0, 1.0, 1.0),"")
...
local SleepyYaw_Edge = menu.Switch("EdgeYaw","Enable EdgeYaw", false, "")
local SleepyYaw_Edge_YawBase_Restore = menu.Combo("EdgeYaw", "Restore YawBase", {"Forward","Backward", "Right","Left","At Target","Freestanding"}, 4, "")
local SleepyYaw_Edge_Condition = menu.MultiCombo("EdgeYaw", "Enemy's Weapon", {"Scout", "AWP", "AutoSniper","Rifle","Pistol"}, 0, "Enemy's Weapon Conditions")
...
SleepyYawMode_Callback()
SleepyYawMode:RegisterCallback(SleepyYawMode_Callback)
...
local function setup_command(C_UserCmd)
    ...
    if g_ClientState.m_choked_commands < chokeValue[5] then --sent
        chokeValue[1] = chokeValue[2]
        chokeValue[2] = chokeValue[3]
        chokeValue[3] = chokeValue[4]
        chokeValue[4] = chokeValue[5]
    end
    chokeValue[5] = g_ClientState.m_choked_commands
...
local function aim_shot_reset()
    if GetHandWeaponID() == 0 or GetHandWeaponID() == nil then
        ...
        if aim_shots_reset == true then
            if (g_GlobalVars.realtime >= aim_nextshots) then
                aim_shots = 0
...
local function FakeLag_Indicator()
    ...
    smooth_fl = smooth_fl + (g_GlobalVars.frametime * 5.99)
    if smooth_fl >= 1 then 
        smooth_fl = 0.01
    end
